...
# april 18-24
channel_id_max=1354
install_carrier_max=1777
install_language_max=297
device_brand_max=1343
plat_os_max=31
...
class BotSpot(torch.nn.Module):
    def __init__(self,edge_index_train,edge_index_test, split_num, data, data_more_feats=None, gbm_models=None,
        ...
        if gbm_models is None and gbm_best_model is None:
            self.linear_local1 = nn.Linear(96, 48)
    ...
    def get_leaf_from_light_gbm(self, left_vertices, right_vertices):
        ...
        if self.gbm_best_model.n_estimators==1: # in case only one decision tree in botspot
        ...
        for i in range(pred_leaf.shape[1]):
            ...
            ret=torch.mean(torch.cat(output_leaf_emb, dim=1),dim=1)[0].to(Device) # leaf node mean pooling
